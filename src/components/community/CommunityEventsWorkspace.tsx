...
export function CommunityEventsWorkspace() {
  ...
  const [draft, setDraft] = useState({ title: "", description: "", city: "", state: "", locationName: "", startDate: "", endDate: "", virtualLink: "" });
  ...
  const load = useCallback(async () => {
    ...
    const [eventsResult, rsvpResult] = await Promise.all([
      supabase.from("community_events")
        ...
        .eq("is_public", true).eq("is_published", true).eq("is_cancelled", false)
    ...
    for (const row of rsvpResult.data ?? []) if (row.event_id && row.status === "going") map.set(row.event_id, row.id);
    setOwnRsvps(map);
    setLoading(false);
  }, [toast, user?.id]);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Listings are submitted by community members and are not independently verified by Civil Rights Hub. Confirm dates, locations, and organizer details before relying on them.</p>
        </div>
      ...
      {showComposer && (
        <Card>
          <CardHeader><CardTitle>Publish a community event</CardTitle><CardDescription>Only publish details you are authorized to share.</CardDescription></CardHeader>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {upcoming.map((event) => {
            ...
            return (
              <Card key={event.id}>
                <CardHeader><CardTitle>{event.title}</CardTitle><CardDescription>{new Date(event.start_date).toLocaleString()}</CardDescription></CardHeader>
