...
CREATE INDEX IF NOT EXISTS idx_activists_verified ON activists(verified);

-- Create index on home_state for location filtering
CREATE INDEX IF NOT EXISTS idx_activists_home_state ON activists(home_state);

-- Create GIN index on focus_areas for array queries
CREATE INDEX IF NOT EXISTS idx_activists_focus_areas ON activists USING GIN(focus_areas);

-- Enable Row Level Security
ALTER TABLE activists ENABLE ROW LEVEL SECURITY;

-- Policy: Anyone can view verified activists
DROP POLICY IF EXISTS "Anyone can view verified activists" ON activists;
...
DROP POLICY IF EXISTS "Authenticated users can submit activists" ON activists;
...
DROP TRIGGER IF EXISTS update_activists_updated_at ON activists;
